...
class MoController < ApplicationController
  def create
    ...
    if params[:body].downcase.start_with?("verify ")
    ...
    elsif params[:body].downcase.start_with?("completed ")
    ...
    elsif params[:body].downcase.start_with?("yes ")
    ...
    elsif params[:body].downcase.start_with?("no ")
      errand = Errand.find(params[:body].split(' ')[1])
      errand_request = ErrandRequest.where(:errand_id => errand.id, :finished => true).first
    ...
    elsif params[:body].downcase.start_with?("search ")
    ...
    elsif params[:body].downcase.start_with?("grab ")
